package org.nasdanika.models.change.ecore;
...
@EPackageNodeProcessorFactory(nsURI = ChangePackage.eNS_URI)
public class EcoreGenChangeProcessorsFactory {

	private Context context;
		
	public EcoreGenChangeProcessorsFactory(Context context) {
		this.context = context;
	}
	
	@EPackageNodeProcessorFactory(
			label = "EMF Change Model",
			documentation =  """
		...
		return new EPackageNodeProcessor(config, context, prototypeProvider) {
			...
			protected void generateDiagramAndGraphActions(Collection<Label> labels, ProgressMonitor progressMonitor) {
				...
				try {
					...
					Files.writeString(new File("target/change.drawio").toPath(), document.save(null));
				} catch (Exception e) {
					e.printStackTrace();
